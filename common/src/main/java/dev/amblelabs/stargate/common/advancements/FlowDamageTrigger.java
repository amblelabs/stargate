...
public class FlowDamageTrigger extends SimpleCriterionTrigger<FlowDamageTrigger.TriggerInstance> {
    ...
    public record TriggerInstance(Optional<ContextAwarePredicate> player, boolean died) implements SimpleInstance {
        ...
        public static Criterion<?> dead() {
            return StargateAdvancementTriggers.FLOW_DAMAGE.get().createCriterion(new TriggerInstance(Optional.empty(), true));
        }
    }
}