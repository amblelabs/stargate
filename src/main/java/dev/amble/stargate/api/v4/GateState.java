...
import dev.amble.stargate.api.v3.NbtDeserializer;
...
public interface GateState<Self extends GateState<Self>> {
    ...
    record StargateChildType<T extends GateState<T>>(Key key, Type type, Group group, NbtDeserializer<T> deser) implements Type<T> {
        ...
        public StargateChildType(Type type, Group group, NbtDeserializer<T> deser) {
            this(new Key(StargateMod.id("stargate/" + type)), type, group, deser);
        ...
        public Identifier id() {
            // todo: cache this
            return StargateMod.id("stargate/" + type);
        }
        ...
        enum Type {
            CLOSED {
                @Override
                public <T extends GateState<T>> StargateChildType<T> get(StargateType<T> t) {
                    return t.closed;
                }
            },
            PRE_OPEN {
            ...
            public abstract <T extends GateState<T>> StargateChildType<T> get(StargateType<T> t);
    ...
    record StargateType<T extends GateState<T>>(
            Key key, StargateChildType<T> closed, StargateChildType<T> preOpen, StargateChildType<T> open) implements Type<T> {

        private static final String TYPE_KEY = "Type";

        public StargateType(StargateChildType<T> closed, StargateChildType<T> preOpen, StargateChildType<T> open) {
            this(new Key(StargateMod.id("stargate")), closed, preOpen, open);
        }
